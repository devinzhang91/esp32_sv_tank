...
void startCameraServer(){
   ...
   httpd_uri_t battery_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        esp_err_t ret;
        ret = httpd_register_uri_handler(camera_httpd, &battery_uri);
        ret = httpd_register_uri_handler(camera_httpd, &index_uri);
        ret = httpd_register_uri_handler(camera_httpd, &go_uri); 
        ret = httpd_register_uri_handler(camera_httpd, &back_uri); 
        ret = httpd_register_uri_handler(camera_httpd, &stop_uri); 
        ret = httpd_register_uri_handler(camera_httpd, &left_uri);
        ret = httpd_register_uri_handler(camera_httpd, &right_uri);
        ret = httpd_register_uri_handler(camera_httpd, &led_uri);
//        ret = httpd_register_uri_handler(camera_httpd, &ledoff_uri);
    }
